...
export function ProcessPathPlaceholders(path: string | RegExp){
    ...
    if (params.length != 0){
        return path.split('/').map(part => part.trim()).filter(part => !!part).map((part) => {
            ...
            return <IRouterFetcherPlaceholder>{ isOptional,
...
export function MatchPath(path: string, info: IRouterFetcherOptimized, params: Record<string, any>, pathParts?: Array<string>){
    ...
    let pathIndex = 0, paramsIndex = 0, lastPlaceholderIndex = { path: -1, params: -1 }, gotoCheckpoint = () => {
        if (lastPlaceholderIndex.path != -1 && lastPlaceholderIndex.params != -1){//Continue from the last optional param
            ...
            delete params![(info.optimized![lastPlaceholderIndex.params] as IRouterFetcherPlaceholder).value];
    ...
    let pathIsMatched = (path: string, targetPath: string) => {
    ...
    for (; pathIndex < pathParts.length && paramsIndex < info.optimized.length;){
        ...
        if (typeof param !== 'string'){
            let accept = <T>(value: T) => {
                params[(param as IRouterFetcherPlaceholder).value] = value;//Add param value
                if ((param as IRouterFetcherPlaceholder).isOptional && lastPlaceholderIndex.path == -1 && lastPlaceholderIndex.params == -1){//Update checkpoint
